...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Actions.Variables
{
    ...
    /// <summary>
    /// Gets the value of a GameObject SharedVariable by name.
    /// </summary>
    [Opsive.Shared.Utility.Description("Gets the value of a GameObject SharedVariable by name.")]
    [Shared.Utility.Category("Variables")]
    public class GetGameObjectVariable : GetVariableBase
    {
        [Tooltip("The name of the GameObject SharedVariable.")]
        [SerializeField] protected SharedVariable<string> m_VariableName;
        [Tooltip("The GameObject variable to store the retrieved value.")]
        [RequireShared] [SerializeField] protected SharedVariable<GameObject> m_Destination;
        ...
        public override TaskStatus OnUpdate()
        {
            var source = GetVariable<GameObject>(m_VariableName);
            if (source != null) {
