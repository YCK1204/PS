...
namespace Opsive.BehaviorDesigner.Runtime.Tasks.Actions.Variables
{
    ...
    /// <summary>
    /// Gets the value of a Vector2 SharedVariable by name.
    /// </summary>
    [Opsive.Shared.Utility.Description("Gets the value of a Vector2 SharedVariable by name.")]
    [Shared.Utility.Category("Variables")]
    public class GetVector2Variable : GetVariableBase
    {
        [Tooltip("The name of the Vector2 SharedVariable.")]
        [SerializeField] protected SharedVariable<string> m_VariableName;
        [Tooltip("The Vector2 variable to store the retrieved value.")]
        [RequireShared] [SerializeField] protected SharedVariable<Vector2> m_Destination;
        ...
        public override TaskStatus OnUpdate()
        {
            var source = GetVariable<Vector2>(m_VariableName);
            if (source != null) {
